...
Initial data is used for server-side hydration. If there is no `initialData`, the required data will be fetched client-side by the `@tulilabs/nft-hooks` library.
...
import {NFTFullPage, FullComponents, NFTDataContext} from '@tulilabs/nft-components';
...
const VerifyHash = () => {
    ...
    return (
        <h3>Verification hash: {nft.data?.contentHash}</h3>
    );
};
